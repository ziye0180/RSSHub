---
title: "RSS 基础 | RSSHub"
source: "https://docs.rsshub.app/zh/joinus/advanced/advanced-feed"
author:
...
本指南面向希望深入了解如何制作 RSS 订阅源的高级用户。如果您是第一次制作 RSS 订阅源，我们建议先阅读 [制作自己的 RSSHub 路由](https://docs.rsshub.app/zh/joinus/new-rss/start-code) 。

一旦您获取了要包含在您的 RSS 订阅源中的数据，就可以将其传递给 `ctx.set('data', obj)` 。然后RSSHub的中间件 [`template.tsx`](https://github.com/DIYgod/RSSHub/blob/master/lib/middleware/template.tsx) 将处理数据并以所需的格式呈现 RSS 输出（默认为RSS 2.0）。除了 [制作自己的 RSSHub 路由](https://docs.rsshub.app/zh/joinus/new-rss/start-code) 中提到的字段外，您还可以使用以下字段进一步自定义 RSS 订阅源。
...
| **`pubDate`**           | *（推荐）* 条目的发布日期，应该 [遵从规范](https://docs.rsshub.app/zh/joinus/advanced/pub-date) 是 [Date object](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Date) | `undefinded` | A, J, R |
...
RSSHub支持制作期刊订阅源。如果用户提供 [通用参数](https://docs.rsshub.app/zh/guide/parameters#%E8%BE%93%E5%87%BA-sci-hub-%E9%93%BE%E6%8E%A5) `scihub` ，则可以将 `item.link` 替换为 Sci-hub 链接。要制作期刊订阅源，您需要在您的 RSS 源中包含一个附加字段：
