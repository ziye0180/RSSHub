---
title: "使用缓存 | RSSHub"
source: "https://docs.rsshub.app/zh/joinus/advanced/use-cache"
author:
...
以上代码片段来自 [制作自己的 RSSHub 路由](https://docs.rsshub.app/zh/joinus/new-rss/start-code) ，展示了如何使用缓存获取每个问题的第一个评论的全文。使用 `cache.tryGet()` 来确定数据是否已经在缓存中。如果不在，则代码会获取数据并将其存储在缓存中。
